...
--
-- $Id: Main.hs,v 1.4 2008-04-30 11:26:54 popeeaco Exp $
--
...
main = do
    ...
    let rf3 = extract_rformula "{[x,y]: 1 <= x && x=y}"
    (ptr_r3, rf3) <- build_relation (["x"], ["y"], rf3)


    eval_relation ptr_r3 rf3
    Omega_stub.relation_print ptr_r3
    Omega_stub.relation_print_with_subs ptr_r3 False

    r <- Omega_stub.relation_print_formula_to_string ptr_r3
    r_string <- peekCString r
    print r_string
